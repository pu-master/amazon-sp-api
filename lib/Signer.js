...
class Signer {
    ...
    if (query){
      ...
      return qs.stringify(query, {
        encoder:(value, defaultEncoder, charset, type) => {
          if (type === 'key'){
            key_is_sku = /^(seller)?sku(s)?$/.test(value.toLowerCase());
            return value;
          }
          // We have to replace whitespace with a '+' sign for SKUs to prevent request from receiving an "Invalid Input" error when SKU includes whitespace
          if (key_is_sku){
  ...
  signAPIRequest(access_token, role_credentials, req_params){
    ...
    if (role_credentials.security_token) {
      headers['x-amz-security-token'] = role_credentials.security_token;
    }
